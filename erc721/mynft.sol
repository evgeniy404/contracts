...
contract TestNFT is ERC721Enumerable, Ownable {
   ...
   constructor(string memory _name, string memory _symbol, address cOwner, string memory uri_, address verifier) Ownable(cOwner) ERC721(_name, _symbol) {
    ...
    function setCoreContract(address _account) public onlyOwner {
        require(_account != address(0), "Address is zero");

        coreContract = _account;
    }
    ...
    function multiCreateFromERC20(address _sender, uint256 _count) external returns(uint256[] memory) {
        require(_msgSender() == coreContract, "Caller is not authorized to use this function");
